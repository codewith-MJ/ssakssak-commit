...
**🔗 서비스 바로가기 → <a href="https://ssakssak-commit.com">ssakssak-commit.com</a>**
...
- [🛠️ 기술 스택](https://github.com/codewith-MJ/ssakssak-commit/wiki/%F0%9F%9B%A0%EF%B8%8F-%EA%B8%B0%EC%88%A0-%EC%8A%A4%ED%83%9D)
  - [🖥️ 프론트엔드](https://github.com/codewith-MJ/ssakssak-commit/wiki/%F0%9F%9B%A0%EF%B8%8F-%EA%B8%B0%EC%88%A0-%EC%8A%A4%ED%83%9D#1-%EF%B8%8F-front-end)
  - [⚙️ 백엔드](https://github.com/codewith-MJ/ssakssak-commit/wiki/%F0%9F%9B%A0%EF%B8%8F-%EA%B8%B0%EC%88%A0-%EC%8A%A4%ED%83%9D#2-%EF%B8%8F-back-end)
  - [📀 데이터](https://github.com/codewith-MJ/ssakssak-commit/wiki/%F0%9F%9B%A0%EF%B8%8F-%EA%B8%B0%EC%88%A0-%EC%8A%A4%ED%83%9D#3--%EB%8D%B0%EC%9D%B4%ED%84%B0)
  - [🤖 AI & 오케스트레이션](https://github.com/codewith-MJ/ssakssak-commit/wiki/%F0%9F%9B%A0%EF%B8%8F-%EA%B8%B0%EC%88%A0-%EC%8A%A4%ED%83%9D#4--ai--%EC%98%A4%EC%BC%80%EC%8A%A4%ED%8A%B8%EB%A0%88%EC%9D%B4%EC%85%98)
  - [🚀 배포](https://github.com/codewith-MJ/ssakssak-commit/wiki/%F0%9F%9B%A0%EF%B8%8F-%EA%B8%B0%EC%88%A0-%EC%8A%A4%ED%83%9D#5--%EB%B0%B0%ED%8F%AC)
...
<pre>
  ├── prisma/                  # Prisma 스키마 (db push 방식, migrations 파일 미사용)
  │   └── schema.prisma
  ...
  │   └── types/               # 전역 타입 정의
  ├── .github/workflows/       # CI/CD (main push → EC2 자동 배포)
  ├── Dockerfile               # web/worker 공용 운영 이미지
  ├── docker-compose.yml       # 로컬 풀스택 실행 (web + worker + redis)
  ├── docker-compose.prod.yml  # 운영 서버 구성 (web + worker + caddy)
  ├── Caddyfile                # 자동 HTTPS 리버스 프록시 설정
  ├── tsconfig.json
  └── package.json
</pre>
...
<img src="https://img.shields.io/badge/Amazon_EC2-FF9900?style=for-the-badge&logoColor=white"> <img src="https://img.shields.io/badge/docker-2496ED?style=for-the-badge&logo=docker&logoColor=white">
...
> [프론트엔드](https://github.com/codewith-MJ/ssakssak-commit/wiki/%F0%9F%9B%A0%EF%B8%8F-%EA%B8%B0%EC%88%A0-%EC%8A%A4%ED%83%9D#1-%EF%B8%8F-front-end) · [백엔드](https://github.com/codewith-MJ/ssakssak-commit/wiki/%F0%9F%9B%A0%EF%B8%8F-%EA%B8%B0%EC%88%A0-%EC%8A%A4%ED%83%9D#2-%EF%B8%8F-back-end) · [데이터](https://github.com/codewith-MJ/ssakssak-commit/wiki/%F0%9F%9B%A0%EF%B8%8F-%EA%B8%B0%EC%88%A0-%EC%8A%A4%ED%83%9D#3--%EB%8D%B0%EC%9D%B4%ED%84%B0) ·[AI & 오케스트레이션](https://github.com/codewith-MJ/ssakssak-commit/wiki/%F0%9F%9B%A0%EF%B8%8F-%EA%B8%B0%EC%88%A0-%EC%8A%A4%ED%83%9D#4--ai--%EC%98%A4%EC%BC%80%EC%8A%A4%ED%8A%B8%EB%A0%88%EC%9D%B4%EC%85%98) · [배포](https://github.com/codewith-MJ/ssakssak-commit/wiki/%F0%9F%9B%A0%EF%B8%8F-%EA%B8%B0%EC%88%A0-%EC%8A%A4%ED%83%9D#5--%EB%B0%B0%ED%8F%AC)

<br />

## 🏗️ 배포 아키텍처 <a id="배포-아키텍처"></a>

AWS EC2 단일 인스턴스 위에 Docker Compose로 운영하며, DB와 Redis는 관리형 서비스를 사용합니다.

```mermaid
flowchart LR
  U["사용자"] -- HTTPS --> C["Caddy<br/>(자동 HTTPS)"]
  ...
  W -- "작업 enqueue" --> R[("Upstash Redis<br/>BullMQ 큐 · 캐시")]
  K -- "작업 consume" --> R
  W --> D[("Supabase<br/>PostgreSQL")]
  K --> D
  K --> G["GitHub API"]
  K --> O["OpenAI API"]
```
...
| 구성 요소                 | 역할                                                                                                               |
| ------------------------- | ------------------------------------------------------------------------------------------------------------------ |
| **EC2 + Docker Compose**  | `web`(Next.js) · `worker`(BullMQ) · `caddy` 3개 컨테이너 운영. web/worker는 동일 이미지를 공유해 빌드는 1회만 수행 |
| **Caddy**                 | 80/443 수신, Let's Encrypt 인증서 자동 발급·갱신 후 web으로 리버스 프록시. web 포트는 외부에 직접 노출하지 않음    |
| **Supabase (PostgreSQL)** | 관리형 DB. 상주 컨테이너 특성에 맞춰 Session pooler(5432)로 연결                                                   |
| **Upstash (Redis)**       | 관리형 Redis(TLS, `rediss://`). BullMQ 큐와 비로그인 리포트 캐시로 사용                                            |

### ⚙️ CI/CD — main push 시 자동 배포

`main` 브랜치에 push되면 GitHub Actions(`.github/workflows/deploy.yml`)가 EC2에 자동 배포합니다.

1. Actions 러너의 공인 IP를 EC2 보안그룹 SSH(22) 인바운드에 **일시 허용** — 평소에는 닫아두는 **동적 보안그룹 패턴**
2. SSH 접속 → `git reset --hard origin/main` → `docker compose -f docker-compose.prod.yml up -d --build`
3. 배포 성공/실패와 관계없이(`if: always()`) 허용했던 러너 IP 제거

> 단일 서버에서 docker build가 동시에 실행되면 메모리가 부족하므로,
> `concurrency` 그룹으로 동시 배포를 방지합니다.

<br />
<br />

## ⚡️ 시작하기 (로컬 실행) <a id="시작하기"></a>
...
| 변수                                        | 설명                                                                    |
| ------------------------------------------- | ----------------------------------------------------------------------- |
| `DATABASE_URL`                              | PostgreSQL 연결 문자열 (DB 컨테이너는 없으므로 외부/관리형 DB 필요)     |
| `REDIS_URL`                                 | Redis 주소 (Docker Compose 실행 시에는 내부 값으로 자동 주입)           |
| `GITHUB_CLIENT_ID` / `GITHUB_CLIENT_SECRET` | GitHub OAuth App 키 (콜백 URL: `{origin}/api/auth/callback/github`)     |
| `NEXTAUTH_URL` / `NEXTAUTH_SECRET`          | 로컬은 `http://localhost:3000` / 시크릿 생성: `openssl rand -base64 32` |
| `OPENAI_API_KEY` / `OPENAI_MODEL`           | 커밋 분석에 사용할 OpenAI 키와 모델                                     |

> 운영 환경변수 템플릿은 `.env.production.example`을 참고하세요.

### 2-A. Docker Compose로 실행 (권장 — 풀스택 한 번에)

```bash
docker compose up --build
```

`web`(http://localhost:3000) + `worker` + `redis`가 함께 실행됩니다.

### 2-B. 개발 모드로 실행

```bash
npm install
docker compose up -d redis   # 로컬 Redis (host 6380 → REDIS_URL=redis://localhost:6380)
npm run dev                  # Next 개발 서버 (http://localhost:3000)
npm run worker:start         # BullMQ 워커 — 별도 터미널 필수 (리포트 생성은 워커가 처리)
```

> ⚠️ 워커를 실행하지 않으면 리포트 생성 요청이 큐에 쌓인 채 진행되지 않습니다.
...
### [💁‍♀️ 김민지](<https://github.com/ssak-three/ssakssak-commit/wiki/%F0%9F%8F%8B%EF%B8%8F%E2%80%8D%E2%99%80%EF%B8%8F-%EA%B8%B0%EB%8A%A5-%EA%B5%AC%ED%98%84-%EB%B0%A9%EC%8B%9D---%EA%B8%B0%EC%88%A0-%EC%B1%8C%EB%A6%B0%EC%A7%80(%EA%B9%80%EB%AF%BC%EC%A7%80)>)
...
![alt text](./public/readme/git_flow.png)
